...
class SubsetModifierListener(HubListener):
    ...
    def __init__(self, state, source_subset, source_data, modify_subset,
                 modify_data, **kwargs):
        ...
        self._source_subset_label = kwargs.get("source_subset_label",
                                               "source_subset") if self._source_subset is None else self._source_subset.label
        self._modify_subset_label = kwargs.get("modify_subset_label",
                                               "modify_subset") if self._modify_subset is None else self._modify_subset.label
    ...
    def _create_subset_state(self, message):
        raise NotImplementedError(
            "Listener has no _create_subset_state method")
    ...
    def _create_modify_subset(self, state):
        if self._use_group:
            ...
            subset = self.modify_data.new_subset(
                ...
                color=self.modify_data.state.color)
        return subset
    ...
    def source_subset(self, value):
        ...
        if self._is_source_group:
            subset = next(
                x for x in value.subsets if x.data == self._source_data)
        else:
            subset = value
