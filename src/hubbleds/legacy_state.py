...
from cosmicds.messages import WriteToDatabaseMessage
...
class LocalState(HubListener):
    ...
    def __post_init__(self):
        ...
        # in the stage state
        # self.step_index.subscribe_change(self._on_step_index_changed)
        # self.step_complete.subscribe_change(self._on_step_complete_changed)
        self.stage_index.subscribe_change(self._on_stage_index_changed)
        self.mc_scoring.subscribe_change(self._update_total_score)
        self.responses.subscribe_change(self.write_to_db)
        ...
        self.has_best_fit_galaxy.subscribe_change(self.update_student_data)

        GLOBAL_STATE.hub.subscribe(
            self,
        ...
        GLOBAL_STATE._glue_app.load_data(
            [
                f"{dataset}.csv"
                for dataset in (
        ...
        galaxies_dict["name"] = [
            x[: -len(self.name_ext)] for x in galaxies_dict["name"]
        ]
        GLOBAL_STATE.data_collection.append(
            Data(label=SDSS_DATA_LABEL, **galaxies_dict)
        )
        ...
        GLOBAL_STATE.data_collection.append(student_measurements)
        GLOBAL_STATE.data_collection.append(student_data)
        GLOBAL_STATE.data_collection.append(class_data)
        for comp in [DISTANCE_COMPONENT, VELOCITY_COMPONENT, STUDENT_ID_COMPONENT]:
            GLOBAL_STATE._glue_app.add_link(
                student_measurements, comp, student_data, comp
            )
            GLOBAL_STATE._glue_app.add_link(
                student_measurements, comp, class_data, comp
            )
        ...
        GLOBAL_STATE.data_collection.append(class_summary_data)
        ...
        for comp in [DISTANCE_COMPONENT, VELOCITY_COMPONENT, STUDENT_ID_COMPONENT]:
            GLOBAL_STATE._glue_app.add_link(
                student_measurements, comp, example_galaxy_meas, comp
            )
            GLOBAL_STATE._glue_app.add_link(
                student_measurements, comp, example_galaxy_meas, comp
            )
        # Make all data writeable
        for data in GLOBAL_STATE.data_collection:
        ...
        self.max_stage_index.subscribe_change(self._on_max_stage_index_changed)

    @debounce(wait=2)
    def write_to_db(self, *args, **kwargs):
        GLOBAL_STATE.hub.broadcast(WriteToDatabaseMessage(self))

    def _modified_import_dict(self, state_dict):
        d = self._modified_dict(state_dict)
    ...
    def _on_stage_index_changed(self, value):
        self.max_stage_index = max(self.max_stage_index.value, value)
        self.write_to_db()
    ...
    def _update_total_score(self, mc_scoring):
        self.total_score = sum(
            mc["score"] or 0 for stage in mc_scoring.values() for mc in stage.values()
        )
        self.write_to_db()
    ...
    def _on_max_stage_index_changed(self, old_value, value):
        if old_value == 4 and value == 5:
            ...
            res = GLOBAL_STATE.request_session.get(
                f"{API_URL}/class-for-student-story/{GLOBAL_STATE.student.id}/hubbles_law"
            ).json()
            ...
            class_data = GLOBAL_STATE.data_collection[CLASS_DATA_LABEL]
            ...
            class_summary_data = GLOBAL_STATE.data_collection[CLASS_SUMMARY_LABEL]
            self.class_data_info = data_summary_for_component(
                class_summary_data, class_summary_data.id[AGE_COMPONENT]
            )
    ...
    def _set_theme(self):
        ...
        v.theme.themes.dark.primary = (
            "colors.blue.darken4"  # Overall theme & header bars
        )
        v.theme.themes.light.primary = "colors.blue.darken3"
        v.theme.themes.dark.secondary = (
            "colors.cyan.darken3"  # Headers on dialogs & buttons that pop up dialogs
        )
        ...
        v.theme.themes.dark.error = (
            "colors.lime.accent1"  # New: Error alerts for free response inputs
        )
        v.theme.themes.light.error = "colors.indigo.darken4"
        v.theme.themes.dark.info = (
            "colors.deepOrange.darken4"  # Instruction scaffolds & viewer highlights
        )
        ...
        v.theme.themes.light.success = (
            "colors.indigo.lighten2"  # formerly green.accent3 or 00E676
        )
        v.theme.themes.dark.warning = (
            "colors.deepOrange.accent4"  # Reflection and some other slideshow headers
        )
    ...
    def _setup_all_data(self):
        ...
        all_json = GLOBAL_STATE.request_session.get(
            f"{API_URL}/{HUBBLE_ROUTE_PATH}/all-data?before={self.started}"
        ).json()
        ...
        all_data = Data(
            ...
            **{
                STATE_TO_MEAS.get(k, k): [x[k] for x in all_measurements]
                for k in all_measurements[0]
            },
        )
        self.prune_none(all_data)
        GLOBAL_STATE.data_collection.append(all_data)
        self.base_all_dict = None

        all_student_summ_data = self.data_from_summaries(
            all_student_summaries,
            label=ALL_STUDENT_SUMMARIES_LABEL,
            id_key=STUDENT_ID_COMPONENT,
        )
        all_class_summ_data = self.data_from_summaries(
            all_class_summaries,
            label=ALL_CLASS_SUMMARIES_LABEL,
            id_key=CLASS_ID_COMPONENT,
        )
        self.make_data_writeable(all_student_summ_data)
        self.make_data_writeable(all_class_summ_data)
        GLOBAL_STATE.data_collection.append(all_student_summ_data)
        GLOBAL_STATE.data_collection.append(all_class_summ_data)
        for comp in [AGE_COMPONENT, H0_COMPONENT]:
            GLOBAL_STATE._glue_app.add_link(
                all_student_summ_data, comp, all_class_summ_data, comp
            )
    ...
    def load_spectrum_data(self, name, gal_type):
        ...
        dc = GLOBAL_STATE.data_collection
        if name not in dc:
    ...
    def setup_example_galaxy(self):
        ...
        example_galaxy_data["name"] = [
            name.replace(".fits", "") for name in example_galaxy_data["name"]
        ]
        GLOBAL_STATE.data_collection.append(
            Data(label=EXAMPLE_GALAXY_DATA, **example_galaxy_data)
        )
        ...
        example_galaxy_seed_data = GLOBAL_STATE.request_session.get(
            f"{API_URL}/{HUBBLE_ROUTE_PATH}/sample-measurements"
        ).json()
        example_galaxy_seed_data = {
            k: np.array([record[k] for record in example_galaxy_seed_data])
            for k in example_galaxy_seed_data[0]
        }
        ...
        indices = indices[1::2][
            :85
        ]  # we need to keep the first 85 so that it always selects the same galaxies "randomly"
        ...
        example_galaxy_seed_data = {
            k: np.array(v)[random_subset] for k, v in example_galaxy_seed_data.items()
        }

        example_galaxy_seed_data[DB_VELOCITY_FIELD] = np.array(
            example_galaxy_seed_data[DB_VELOCITY_FIELD],
            dtype=type(example_galaxy_seed_data[DB_VELOCITY_FIELD][0]),
        )
        example_galaxy_seed_data = Data(
            label=EXAMPLE_GALAXY_SEED_DATA, **example_galaxy_seed_data
        )

        GLOBAL_STATE.data_collection.append(example_galaxy_seed_data)

        # Create empty Data for student measurements of example galaxy
        single_gal_student_cols = [
            SAMPLE_ID_COMPONENT,
            NAME_COMPONENT,
            RA_COMPONENT,
            DEC_COMPONENT,
        ...
        empty_record[RESTWAVE_COMPONENT] = (
            H_ALPHA_REST_LAMBDA
            if ("H" in example_galaxy_data[ELEMENT_COMPONENT][0])
            else MG_REST_LAMBDA
        )
        ...
        component_dict = {
            c: list(example_galaxy_measurements[c]) for c in main_components
        }
        ...
        GLOBAL_STATE.data_collection.append(example_galaxy_measurements)
        self.add_new_row(
            data=example_galaxy_measurements,
            changes={MEASUREMENT_NUMBER_COMPONENT: "second"},
        )
    ...
    def add_new_row(self, dc_name=None, data=None, changes={}):
        ...
        if dc_name is None:
            ...
            data = GLOBAL_STATE.data_collection[dc_name]
            return
    ...
    def update_student_data(self, *args):
        ...
        all_students_summ_data = GLOBAL_STATE.data_collection[
        ...
        index = next(
            (
                i
                for i in range(all_students_summ_data.size)
                if all_students_summ_data[STUDENT_ID_COMPONENT][i] == student_id
            ),
            None,
        )
        if index is None:
        ...
        if self.has_best_fit_galaxy:
            ...
            subset = next(
                (s for s in student_data.subsets if s.label == BEST_FIT_SUBSET_LABEL),
                None,
            )
            if subset is not None:
                subset.subset_state = subset_state
            else:
                student_data.new_subset(
                    label=BEST_FIT_SUBSET_LABEL,
                    subset=subset_state,
                    color="blue",
                    alpha=1,
                    markersize=58,
    ...
    def update_example_galaxy_data(self, *args):
        ...
        self.make_data_writeable(example_galaxy_data)
    ...
    def prune_none(data):
        ...
        pruned_components = {
            comp.label: [data[comp][x] for x in keep] for comp in data.main_components
        }
    ...
    def data_from_measurements(self, measurements, sample_measurements=False):
        ...
        components = {
            STATE_TO_MEAS.get(k, k): [
                measurement.get(k, None) for measurement in measurements
            ]
            for k in DB_MEASUREMENT_FIELDS
        }
        if sample_measurements:
            sample_components = {
                ...
                for k in DB_SAMPLE_MEASUREMENT_FIELDS
            }
    ...
    def data_from_summaries(self, summaries, id_key=None, label=None):
        components = {
            STATE_TO_SUMM.get(k, k): [summary.get(k, None) for summary in summaries]
            for k in DB_SUMMARY_FIELDS
        }
    ...
    def fetch_measurement_data_and_update(
        self,
        url,
        label,
        prune_none=False,
        ...
        new_data = self.data_from_measurements(
            measurements, sample_measurements=sample_measurements
        )
        ...
        if label == EXAMPLE_GALAXY_MEASUREMENTS:
            ...
            if new_data.size == 1:
                ...
                empty_row[DB_MEASNUM_FIELD] = (
                    "second" if new_data[DB_MEASNUM_FIELD][0] == "first" else "first"
                )
                empty_row.update(
                    {
                        k: None
        ...
        data = GLOBAL_STATE.data_collection[label]
        data.update_values_from_data(new_data)
        if make_writeable:
            self.make_data_writeable(data)
    ...
    def update_summary_data(self, measurements, summ_label, id_field):
        ...
        components = {H0_COMPONENT: hubbles, AGE_COMPONENT: ages}
        components[id_field] = list(ids)
        new_data = Data(label=summ_label, **components)

        data = GLOBAL_STATE.data_collection[summ_label]
        data.update_values_from_data(new_data)

    def fetch_student_data(self):
        student_meas_url = (
            f"{API_URL}/{HUBBLE_ROUTE_PATH}/measurements/{self.student_user['id']}"
        )
        self.fetch_measurement_data_and_update(
            student_meas_url, STUDENT_MEASUREMENTS_LABEL, make_writeable=True
        )
    ...
    def _check_class_data_need_update(self, measurements):
        last_modified = max(
            [isoparse(m[DB_LAST_MODIFIED_FIELD]) for m in measurements], default=None
        )
        need_update = (
            self.class_last_modified is None
            or last_modified is None
            or last_modified > self.class_last_modified
        )
    ...
    def fetch_class_data(self):
        class_data_url = f"{API_URL}/{HUBBLE_ROUTE_PATH}/stage-3-data/{GLOBAL_STATE.student.id.value}/{GLOBAL_STATE.classroom.id.value}"
        ...
        updated_meas, updated_data = self.fetch_measurement_data_and_update(
            class_data_url,
            CLASS_DATA_LABEL,
            prune_none=True,
            update_if_empty=False,
            check_update=self._check_class_data_need_update,
        )

        if updated_data is not None:
            class_id = GLOBAL_STATE.classroom.id.value
            self.update_summary_data(
                updated_data, CLASS_SUMMARY_LABEL, STUDENT_ID_COMPONENT
            )
            if ALL_DATA_LABEL in GLOBAL_STATE.data_collection:
                all_data = GLOBAL_STATE.data_collection[ALL_DATA_LABEL]
                ...
                if self.base_all_dict is None:
                    indices = (
                        all_data[CLASS_ID_COMPONENT] != GLOBAL_STATE.classroom.id.value
                    )
                    self.base_all_dict = {
                        k.label: all_data[k][indices] for k in all_data.main_components
                    }

                all_dict = self.base_all_dict.copy()
                all_dict[CLASS_ID_COMPONENT] = np.concatenate(
                    [all_dict[CLASS_ID_COMPONENT], [class_id] * len(updated_meas)]
                )
                for k in all_dict:
                    if k == CLASS_ID_COMPONENT:
                        continue
                    all_dict[k] = np.concatenate(
                        [
                            all_dict[k],
                            [m[MEAS_TO_STATE.get(k, k)] for m in updated_meas],
                        ]
                    )
            ...
            if ALL_CLASS_SUMMARIES_LABEL in GLOBAL_STATE.data_collection:
                dists = updated_data[DISTANCE_COMPONENT]
                vels = updated_data[VELOCITY_COMPONENT]
                h0, age = self.create_single_summary(dists, vels)
                all_summ_data = GLOBAL_STATE.data_collection[ALL_CLASS_SUMMARIES_LABEL]
                index = next(
                    (
                        i
                        for i in range(all_summ_data.size)
                        if all_summ_data[CLASS_ID_COMPONENT][i] == class_id
                    ),
                    None,
                )
                if index is None:
