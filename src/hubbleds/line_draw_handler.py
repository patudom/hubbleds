...
class LineDrawHandler:
    """
    This class handles the interactions for allowing a student to draw a line
    on a glue-jupyter scatter viewer.
    """
    ...
    def __init__(self, app, viewer):
        ...
        self._interaction = MouseInteraction(x_scale=scales_image['x'],
    ...
    def _handle_mousemove(self, data):
        ...
        # Note that, since the image scales of the glue-jupyter scatter viewer
        # are from 0 to 1 we don't need to worry about any normalization
        # If this weren't the case, we could grab the normalized coordinates
        # via:
        ...
        if self._drawn_line is None:
            self._drawn_line = LinesGL(x=[0, self._viewer.state.x_max],
                                       y=[0, 0], scales=image.scales,
                                       colors=['black'])
    ...
    def _handle_click(self, data):
        if self._follow_cursor:
            ...
            domain = data['domain']
            x, y = domain['x'], domain['y']
            figure.marks = [mark for mark in figure.marks if
                            mark is not self._endpt]
    ...
    def _coordinates_in_bounds(self, x, y):
        """
        If a student drags the endpoint beyond the viewer bounds, we want to
        bring it back inside. This function, given x and y coordinates of a
        chosen endpoint, finds the coordinates of the point where their line
        crosses the boundary of the viewer.
        """
    ...
    def clear(self):
        if self._drawn_line is None and self._endpt is None:
            return
        figure = self._viewer.figure
        to_remove = [x for x in [self._drawn_line, self._endpt] if
                     x is not None]
